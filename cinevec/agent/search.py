...
from sqlalchemy import bindparam, desc, func, select, text, Select
...
def filter_search(
    ...
    **filters
    ...
    order = (Movie.weighted_rating.desc().nulls_last()
            if sort_by_rating else Movie.year.desc().nulls_last())
...
def hybrid_search(
    ...
    **filters
    ...
    text side (see text_search); rrf_k tunes the fusion itself."""
    ...
    stmt = text(f"""
        WITH text_hits AS (
            SELECT m.id, row_number() OVER (
                ORDER BY ts_rank(:weights, m.search_doc,
                                 websearch_to_tsquery('english', :q), :norm) DESC
            ) AS rank
            FROM movies m
            WHERE m.search_doc @@ websearch_to_tsquery('english', :q) {fsql}
            LIMIT :pool
        ),
        vec_hits AS (
            SELECT m.id, row_number() OVER (
                ORDER BY m.embedding <=> :qvec
            ) AS rank
            FROM movies m
