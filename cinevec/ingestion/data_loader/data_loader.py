...
from cinevec.utils.file_utils import create_path, path_exists
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    VOTE_FLOOR_M = np.median(
        df.vote_count
    )  # median vote count across all movies
    ...
    def calc_weighted_rating(row):
        ...
        return np.round(
            (vote_count / (vote_count + VOTE_FLOOR_M)) * vote_avg
            + (VOTE_FLOOR_M / (vote_count + VOTE_FLOOR_M)) * C,
            3,
        )
    ...
    df_clean = df.assign(
        ...
        genres=df.genre.str.split(",").apply(
...
def load_and_store_data(
    config: ConfigBox, sample_n: int | None = None
) -> pd.DataFrame:
    """
    ...
    if path_exists(output_path):
        logger.info(
            f"Data file already exists at {output_path}. Loading it instead of downloading."
        )
        ...
        df_clean = pd.read_csv(
            output_path, converters={"genres": ast.literal_eval}
        )
