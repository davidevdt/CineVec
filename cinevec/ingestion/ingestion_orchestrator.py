...
from cinevec.ingestion.db.build_rag_db import create_schema, get_engine
...
def orchestrate_ingestion(
    rebuild: bool = False, sample_n: int | None = None
) -> None:
    ...
    logger.info(
        "Database schema created successfully. Starting ingestion (this might take take up to ~5 minutes)..."
    )
    ingest(engine, movie_items, config)
    logger.info("Ingestion process completed successfully.")

    logger.info("Number of rows in the movies table:")
